...
HDMITX_SetOutput()
{
    ...
    if( bHDMIMode )
    {
        OS_PRINTF("ConfigAVIInfoFrame, VIC=%d, TMDSClock = %d\n", VIC, TMDSClock);
           ...
           if( !bValidCEA && EDID_Buf[0] == 0x2 && EDID_Buf[1] == 0x3 )  //EDID_Buf[0] == 0x2  ==> Additional timing data type 2
            {
                ...
                iprintf("ParseCAEDID: %d, IEEEOUI: %04x\n", err, RxCapability.IEEEOUI);
                if( !err )
                {
 
				    if(RxCapability.IEEEOUI==0x0c03 || RxCapability.IEEEOUI==0x0c45dd8)
				    {
